...
type ProblemDescriptionProps = {
  problem: Problem;
  _solved: boolean; //local solved state i.e not from db
};
...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({
  ...
  //in this page, we are only going to have one problem
  const { currentProblem, loading, problemDifficultyClass, setCurrentProblem } =
    useGetCurrentProblem(problem.id); //problem.id to know which data should be fetched
  const { liked, disliked, solved, setData, starred } =
    useGetUsersDataOnProblem(problem.id);
  //state for updating
  const [updating, setUpdating] = useState(false);

  //getiing user and problem data
    ...
    await runTransaction(firestore, async (transaction) => {
      ...
      if (userDoc.exists() && problemDoc.exists()) {
        if (liked) {
          ...
          //if disliked, similar functionality as above
          transaction.update(userRef, {
          ...
          <div className="w-full">
            ...
            {!loading && currentProblem && (
              <div className="flex items-center mt-3">
                ...
                {/* if user has solved the problem */}
                {(solved || _solved) && (
...
//our function to get current problem
function useGetCurrentProblem(problemId: string) {
  //variable for current problem, empty initially
  ...
  useEffect(() => {
    ...
    const getCurrentProblem = async () => {
      ...
      //similar to before for mutilple documents but this time we only want one document

      //getting the document reference from collection of problems from firestore db
      const docRef = doc(firestore, "problems", problemId);
      //getting the particular document
      const docSnap = await getDoc(docRef);
      if (docSnap.exists()) {
        //if doc exists, get its  data
        const problem = docSnap.data();

        //set the data in the above declared variable
...
function useGetUsersDataOnProblem(problemId: string) {
  ...
  useEffect(() => {
    const getUsersDataOnProblem = async () => {
      //getting the user reference in our firestore db's user's collection with user id
      const userRef = doc(firestore, "users", user!.uid);
      const userSnap = await getDoc(userRef);
      if (userSnap.exists()) {
        //if it exists then get its data
        const data = userSnap.data();
        //destructure the data we want
        const {
          solvedProblems,
          likedProblems,
          dislikedProblems,
    ...
    //when we go on some other problem, it clears the set data and re runs the function
