...
const Timer: React.FC<TimerProps> = () => {
  //making a state function to show the timer
  const [showTimer, setShowTimer] = useState<boolean>(false);

  //making a state function to show the time
  const [time, setTime] = useState<number>(0);

  //converts the time number into a string to display
  ...
  useEffect(() => {
    //when show timer is true, useEffect runs
    let intervalId: NodeJS.Timeout;

    //when show timer is true, with every passing second our timer gets incremented by 1 and whenever time changes, our component below gets re-rendered since we are passing it as a parameter
    if (showTimer) {
  ...
  return (
    <div>
      {/* if show timer is true then show ticking timer else show an svg*/}
      {showTimer ? (
        ...
        <div
          ...
          //when we click the clock icon, it shows the timer
